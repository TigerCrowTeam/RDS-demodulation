...
# -*- coding: utf-8 -*-
"""
Created on Wed May 21 15:48:30 2025
...
def read_gr_constellation_hard_bits_file(filepath: str) -> np.ndarray:
    """
    Reads a .bin file saved by GNU Radio's File Sink from a Constellation Decoder
    (configured for hard bits output).

    Assumes each byte in the file directly represents one decoded bit (0x00 for 0, 0x01 for 1).

    Args:
        filepath (str): The path to the .bin file (e.g., "test1.bin").

    Returns:
        np.ndarray: A 1D NumPy array of dtype uint8, containing 1s and 0s.
                    Returns an empty array if the file is empty or cannot be read.
    """
    try:
        # np.fromfile is efficient for reading raw binary data
        # dtype=np.uint8 tells NumPy to interpret each byte as a uint8
        data = np.fromfile(filepath, dtype=np.uint8)

        # Basic validation: ensure values are indeed 0 or 1
        if not np.all(np.isin(data, [0, 1])):
            print(f"Warning: File '{filepath}' contains values other than 0 or 1.")
            # Depending on your needs, you might want to:
            # 1. Clip/saturate values to 0/1: data[data != 0] = 1 (or similar)
            # 2. Raise an error
            # For now, we'll just return it as is, but you've been warned.
...
bitsRaw=read_gr_constellation_hard_bits_file("outputRDS1.bin")
bitsRaw=bitsRaw.astype(np.int8)


#Manchester
bitsManch=bitsRaw[1:]-bitsRaw[:-1]

bits=np.empty([], dtype=np.int8)

checker=1
for x in range(len(bitsManch-1)):
    if checker==1:
        if bitsManch[x]==0:
            checker=1
        else:
            bits=np.append(bits,(bitsManch[x]+1)/2)
            checker=0
    else:
        checker=1
